...
gem 'aws-sdk-s3', '~> 1.85', '>= 1.85.0' # AWS S3 module, (aws-sdk v3) - Swapped with aws-sdk, as newest version will pull in ALL modules. If other modules needed, require those separately.
...
gem 'twilio-ruby', '~> 5.34.1' #used to send text messages for 2FA and other text messaging requiremments
...
gem 'mechanize', '2.7.7' #used by ach processor and fedbank processor to programatically interact with websites
gem 'oauth', '0.4.7' #used by API to do oauth
gem 'doorkeeper', '5.1.0' # used by APIs to handle oauth2 provider functionality
gem 'doorkeeper-openid_connect', '1.8.2' # implements an openid connect authentication on top of doorkeeper gem
...
gem 'jira-ruby', '>= 1.0.0', require: ['jira'] #make tickets in JIRA, used by compliance
...
gem 'faye-websocket', '>= 0.11.1' #used by walrus
...
group :development, :test do
  ...
  gem 'annotate', '~> 3.0.0' #provides annotation at top of files by running annotate
  ...
  gem 'meta_request', '>= 0.8.0' # Required in order to use rails panel https://github.com/dejan/rails_panel
...
group :development, :test, :staging, :sandbox do
  gem 'factory_bot', '~> 4.11.0', require: false # factories for testing and preconfigs
end
